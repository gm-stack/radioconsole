...
class DockerStatus(LogViewer):
    ...
    def setup_logviewer(self, service):
        self.set_tail_command(f"journalctl --no-hostname -f CONTAINER_NAME={service}")
    ...
    def update(self, dt):
        if self.data_updated:
            ...
            for unit_name in self.config.services:
                ...
                description = re.sub(f'^{self.config.remove_prefix}','', description) # we are stuck on python 3.7 at the moment
                description = re.sub(f'{self.config.remove_suffix}$','', description) # so no trimsuffix trimprefix
                ...
                if service.get('load') == 'loaded':
                    button_text = "Start" if service.get('active') in ('exited','dead','paused','created') else 'Stop'
                else:
                    button_text = "---"
                ...
                if service:
                    count += 1
                if service.get('active') == "running":
                    running += 1
                if service.get('active') == 'restarting':
                    activating += 1
                if service.get('active') in ['exited','dead','paused']:
                    errored += 1
    ...
    def process_events(self, e):
        ...
        if e.type == pygame.USEREVENT and e.user_type == pygame_gui.UI_BUTTON_PRESSED:
            if e.ui_element in self.ui_element_start_stop_buttons and e.ui_element.text != "---":
                service = self.ui_element_start_stop_buttons[e.ui_element]
                self.setup_logviewer(service)
                def run_cmd(ts):
                    res = self.run_command(ts, f"sudo docker {e.ui_element.text.lower()} {service}")
    ...
    def do_fetch_service_info(self,ts,services):
        ...
        docker_ps = run_command(' docker ps --format json -a')
        docker_containers = {}
        for line in docker_ps.split("\n"):
            container_details = json.loads(line)
            docker_containers[container_details['Names']] = container_details
