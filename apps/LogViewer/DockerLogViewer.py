...
class DockerLogViewer(LogViewer):
    default_config = {
        "port": 22,
        "username": "pi",
        "lookback": 100,
    ...
    def run_tail_command(self, ts, _):
        self.status_message(f"tail docker logs for {self.config.services}\n")

        start_timestamp = None
        service_flags = " ".join([f"CONTAINER_NAME={s}" for s in self.config.services])
    ...
    def filter(self, json_text):
        ...
        for json_obj in json_text.split("\n"):
            ...
            out_msg = DockerLogMessage(j.items(), json_obj, self.show_service_name)
